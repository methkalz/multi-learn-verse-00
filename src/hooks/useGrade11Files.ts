...
export const useGrade11Files = () => {
  const [documents, setDocuments] = useState<Grade11Document[]>([]);
  ...
  const fetchDocuments = async () => {
    try {
      ...
      setDocuments((data || []) as Grade11Document[]);
    } catch (error) {
      logger.error('Error fetching Grade 11 documents', error as Error);
  ...
  const fetchVideos = async () => {
    try {
      ...
      if (error) throw error;
      setVideos((data || []) as Grade11Video[]);
    } catch (error) {
      logger.error('Error fetching Grade 11 videos', error as Error);
  ...
  const addDocument = async (documentData: Omit<Grade11Document, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      const { data, error } = await supabase
        .from('grade11_documents')
        .insert(documentData)
      ...
      setDocuments(prev => [data as Grade11Document, ...prev]);
  ...
  const addDocuments = async (documentsData: Omit<Grade11Document, 'id' | 'created_at' | 'updated_at'>[]) => {
    try {
      const { data, error } = await supabase
        .from('grade11_documents')
  ...
  const addVideo = async (videoData: Omit<Grade11Video, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      const { data, error } = await supabase
        .from('grade11_videos')
        .insert(videoData)
      ...
      setVideos(prev => [data as Grade11Video, ...prev]);
  ...
  const updateDocument = async (id: string, updates: Partial<Grade11Document>) => {
    try {
      const { data, error } = await supabase
        .from('grade11_documents')
        .update(updates)
      ...
      setDocuments(prev => prev.map(doc => doc.id === id ? data as Grade11Document : doc));
  ...
  const updateVideo = async (id: string, updates: Partial<Grade11Video>) => {
    try {
      const { data, error } = await supabase
        .from('grade11_videos')
        .update(updates)
      ...
      setVideos(prev => prev.map(video => video.id === id ? data as Grade11Video : video));
