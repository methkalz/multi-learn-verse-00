...
// Types للمهام الافتراضية للصف العاشر
interface Grade10DefaultTask {
  id: string;
  phase_number: number;
  phase_title: string;
  task_title: string;
...
interface Grade10StudentTaskProgress {
  id: string;
  student_id: string;
...
interface TaskPhase {
  phase_number: number;
  phase_title: string;
  tasks: (Grade10DefaultTask & { progress?: Grade10StudentTaskProgress })[];
  completed_count: number;
  total_count: number;
  completion_percentage: number;
}

export const useGrade10DefaultTasks = () => {
  const { userProfile } = useAuth();
  const [phases, setPhases] = useState<TaskPhase[]>([]);
  const [loading, setLoading] = useState(true);
  const [allTasks, setAllTasks] = useState<Grade10DefaultTask[]>([]);
  const [studentProgress, setStudentProgress] = useState<Grade10StudentTaskProgress[]>([]);
  ...
  const organizeTasksByPhases = (tasks: Grade10DefaultTask[], progress: Grade10StudentTaskProgress[]): TaskPhase[] => {
