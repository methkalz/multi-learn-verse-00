...
interface Grade11BulkDocumentUploadProps {
...
const Grade11BulkDocumentUpload: React.FC<Grade11BulkDocumentUploadProps> = ({
  onUploadComplete,
  onCancel,
  uploadFile,
  addDocuments
  ...
  const [globalSettings, setGlobalSettings] = useState({
    category: 'worksheets' as 'bagrut_exam' | 'worksheets' | 'exams' | 'materials',
    is_visible: true,
    ...
    try {
      ...
      for (const fileData of files) {
        try {
          ...
          const bucketPath = `grade11/${safeStoragePath}`;
          
          logger.debug('Processing Grade 11 file upload', {
      ...
      logger.error('Upload error during Grade 11 bulk document upload', error as Error);
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            رفع ملفات متعددة - الصف الحادي عشر
          </CardTitle>
        </CardHeader>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <div className="flex items-center justify-center">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => updateFile(fileData.id, { is_visible: !fileData.is_visible })}
                          >
                            {fileData.is_visible ? (
                      ...
                      {uploading && (
                        <div className="space-y-2">
                          ...
                          <div className="flex justify-between text-sm">
                            <span>
                              {fileData.uploaded ? 'مكتمل' : fileData.error ? 'خطأ' : 'جاري الرفع...'}
                            </span>
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          <span>{fileData.originalFileName}</span>
                          <span>•</span>
                          <span>{(fileData.file.size / 1024 / 1024).toFixed(2)} MB</span>
                        </div>
                        
                        <div className="flex items-center gap-2 text-xs bg-green-50 text-green-700 p-2 rounded border">
                          <Check className="h-3 w-3" />
                          <span>سيتم عرض الاسم: "{fileData.title}" (الاسم الأصلي محفوظ)</span>
                        </div>
                      </div>
                    </div>
                  </Card>
...
export default Grade11BulkDocumentUpload;