-- أولاً: تصحيح القيم المسموحة في left_type و right_type
-- إزالة القيود القديمة وإضافة قيود جديدة تسمح بـ 'term' و 'definition'

-- إزالة القيود الحالية
ALTER TABLE pair_matching_pairs DROP CONSTRAINT IF EXISTS pair_matching_pairs_left_type_check;
ALTER TABLE pair_matching_pairs DROP CONSTRAINT IF EXISTS pair_matching_pairs_right_type_check;

-- إضافة قيود جديدة تسمح بالقيم المطلوبة
ALTER TABLE pair_matching_pairs 
ADD CONSTRAINT pair_matching_pairs_left_type_check 
CHECK (left_type IN ('text', 'term', 'definition', 'concept', 'image'));

ALTER TABLE pair_matching_pairs 
ADD CONSTRAINT pair_matching_pairs_right_type_check 
CHECK (right_type IN ('text', 'term', 'definition', 'concept', 'image'));

-- الآن إضافة الأزواج الجديدة
